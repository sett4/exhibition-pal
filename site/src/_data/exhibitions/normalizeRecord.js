...
function indexArtworksByExhibition(artworks = []) {
  ...
  artworks.forEach((artwork) => {
    ...
    const bucket = grouped.get(artwork.exhibitionId) ?? {
      items: [],
      ids: new Set(),
      used: false,
    };
...
export async function normalizeSheet(sheetResponse, options = {}) {
  ...
  if (values.length <= 1) {
    return {
      ...
      duplicateArtworkIds: [],
      artworkSortKey: "artworkId",
    };
  }

  const {
    grouped: artworksByExhibition,
    duplicateIds,
    warnings: artworkWarnings,
  } = indexArtworksByExhibition(options.artworks ?? []);
  ...
  for (let rowIdx = 0; rowIdx < rows.length; rowIdx += 1) {
    ...
    const relatedUrls = buildRelatedUrls(
  ...
  const metadataSortKey =
    readCell(headerRow, headerIndex, "artworkSortKey") || "artworkId";
